    ...
    return <div className={ cls.container }>
        <div className={ cls.horizontalScrollingItems }>
            { categoryNames.map((category, index) => {
                return <Link href={ `/products/category/${ categories[category].url }` } key={ index }>
                    <figure className={ cls.horizontalScrollingItem }>
                        <Image className={ cls.horizontalScrollingImage }
                               src={ `/img/categories/${ categories[category].icon }` }
                               alt={ categories[category].name } width={ 200 } height={ 200 }/>
                        <figcaption className={ cls.figcaption }>{ t(`category.${ category }`) }</figcaption>
                    </figure>
                </Link>
            })
            }
